...
## Architecture
- Screen: app/(tabs)/reels.tsx
- Hook: src/hooks/useReelsFeed.ts
- Types:
  - src/types/reels.ts

## Data Source
- Reels dataset from hook layer (mock-backed in current implementation).
- Per-session like state managed locally in screen state.

## UI Structure
1. Full-screen reel canvas
- Each reel occupies viewport height.
...
## Interactions
- Like toggle updates local liked map keyed by reel id.
