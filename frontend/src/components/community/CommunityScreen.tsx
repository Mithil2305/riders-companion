...
export function CommunityScreen() {
  ...
  const { activeRide, suggestedGroups, nearbyRides, myRides } = useCommunityData();
      ...
      if (item === 'suggested') {
        return (
          <View style={styles.sectionWrap}>
            <SectionHeader actionLabel="VIEW ALL" onPressAction={() => {}} title="Suggested Groups" />
            <ScrollView
            ...
            >
              {suggestedGroups.map((group) => (
                <SuggestedGroupCard item={group} key={group.id} onPress={() => {}} />
              ))}
            </ScrollView>
          </View>
        );
          ...
          <View style={styles.sectionWrap}>
            ...
            <View style={styles.rideListWrap}>
              {nearbyRides.map((ride: RideItem) => (
                <RideCard item={ride} key={ride.id} mode="nearby" onPrimaryAction={() => {}} />
              ))}
            </View>
          </View>
        );
        ...
        <View style={styles.sectionWrap}>
          ...
          <View style={styles.rideListWrap}>
            {myRides.map((ride: RideItem) => (
              <RideCard item={ride} key={ride.id} mode="myRides" onPrimaryAction={() => {}} />
            ))}
          </View>
        </View>
      );
  ...
  return (
    <SafeAreaView edges={['left', 'right', 'top']} style={styles.container}>
      <Header onBack={() => router.back()} onStartRide={() => router.push('/(tabs)/ride')} />

      <FlatList
        ListHeaderComponent={<ActiveRideCard data={activeRide} onOpenRide={() => router.push('/tracking')} />}
